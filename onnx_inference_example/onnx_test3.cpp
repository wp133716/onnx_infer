...
int main()
{
    ...
    // 查看是否支持onnxruntime-gpu，打印出支持的provider
    std::cout << "Supported backends:\n";
    for (const auto& it : Ort::GetAvailableProviders()) {
        std::cout << it << "\n";
    }
    // OrtCUDAProviderOptions cuda_options;
    // cuda_options.device_id = 0;
    // session_options.AppendExecutionProvider_CUDA(cuda_options);
    // // session_options.AppendExecutionProvider_TRT();
    ...
    // 设置onnxruntime-gpu为backend,并设置gpu id,需要头文件<onnxruntime/core/session/onnxruntime_cxx_api.h>和<onnxruntime/core/providers/cuda/cuda_provider_factory.h>
    // session_options.AppendExecutionProvider_CUDA(0); //cannot convert ‘int’ to ‘const OrtCUDAProviderOptions&’
    // session_options.AppendExecutionProvider_TRT();
    // session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    // session_options.SetIntraOpNumThreads(1);
    // session_options.SetInterOpNumThreads(1);
    ...
    // const char* model_path = "../test-32.onnx";
    const char* model_path = "../tf_model_10_x100.onnx";
    ...
    // 打开txt文件，读取数据，txt文件中数据为一行，以空格分隔
    std::ifstream infile;
    infile.open("/home/user/temporary/0504result/1.txt");
    if(!infile.is_open())
    {
    ...
    {
        std::stringstream ss(s); //使用串流实现对string的分割,需要头文件<sstream>：#include <sstream>
        double temp;
        while(ss>>temp)
        {
    ...
    {
        std::cout<<setprecision(10)<<fileData[i]<<" ";
        if ((i+1)%4 == 0 && i!=0)
            std::cout<<std::endl;
    }
    ...
    {   
        ...
        for (unsigned int i = 0; i < input_tensor_size; i++)
            input_tensor_values[i] = fileData[i]; //float(virtual_image[i]);
        ...
        for (int i=0; i<outputShape[0]; i++)
            for(int j=0; j<outputShape[1]; j++)
            {
                for(int k=0; k<outputShape[2]; k++)
                    std::cout<<setprecision(10)<<floatarr[i*outputShape[1]*outputShape[2]+j*outputShape[2]+k]/100<<" ";
                std::cout<<std::endl;
            }
        for (int i=0; i<outputShape[0]; i++)
            for(int j=0; j<outputShape[1]; j++)
            {
                for(int k=0; k<outputShape[2]; k++)
                    std::cout<<setprecision(10)<<floatarr[i*outputShape[1]*outputShape[2]+j*outputShape[2]+k]<<" ";
                std::cout<<std::endl;
            }
